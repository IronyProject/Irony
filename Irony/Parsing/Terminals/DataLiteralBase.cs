...
namespace Irony.Parsing {
  ...
  //DateLiteralBase is a base class for a set of specialized terminals with a primary purpose of building data readers
  ...
  public class DataLiteralBase : Terminal {
    ...
    protected virtual object ConvertValue(ParsingContext context, string textValue) {
      switch(DataType) {
        ...
        case TypeCode.DateTime: return DateTime.ParseExact(textValue, DateTimeFormat, Culture);
        case TypeCode.Single:
        case TypeCode.Double:
          var dValue = Convert.ToDouble(textValue, Culture); 
          if (DataType == TypeCode.Double)  return dValue; 
          return Convert.ChangeType(dValue, DataType, Culture);

        default: //integer types
          var iValue = (IntRadix == 10)? Convert.ToInt64(textValue, Culture) : Convert.ToInt64(textValue, IntRadix);
          if (DataType == TypeCode.Int64)  return iValue; 
          return Convert.ChangeType(iValue, DataType, Culture);
      }
    }//method
