...
namespace Irony.Parsing {
  ...
  public class CodeOutlineFilter : TokenFilter {
    ...
    public override IEnumerable<Token> BeginFiltering(ParsingContext context, IEnumerable<Token> tokens) {
      ...
      //  not by terminal. 
      context.Parser.Language.Grammar.WhitespaceChars = " \t\r\n\v"; 
      _prevLine = 0;
      _indents.Clear();
      foreach (Token token in tokens) {
        ...
        if (currIndent > prevIndent) {
          ...
          if (_indents.Count == 0 || _indents.Peek() != currIndent) {
            yield return base.CreateErrorTokenAt(token.Location, "Invalid dedent level, no previous matching indent found.");
